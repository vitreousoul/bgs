...
typedef enum
{
    ...
    White_Queen_Side_Castle_Flag = (1 << 1),
    White_King_Side_Castle_Flag  = (1 << 2),
    Black_Queen_Side_Castle_Flag = (1 << 3),
...
typedef struct
{
    ...
    traversal_node Nodes[Game_Tree_Node_Pool_Size];
} traversal;
...
typedef struct
{
    ...
    traversal_node DebugTraversalNodes[Game_Tree_Node_Pool_Size];
...
internal void ClearTraversals(app_state *AppState, traversal_node *TraversalNodes)
{
    for (u64 I = 0; I < Game_Tree_Node_Pool_Size; ++I)
    {
        TraversalNodes[I].Visited = 0;
    }
}
...
internal void SetTraversalAsVisited(traversal_node *TraversalNodes, s32 TraversalIndex)
{
    ...
    {
        TraversalNodes[TraversalIndex].Visited = 1;
    }
}
...
internal s32 Debug_WalkGameTreeAndReturnCount_(app_state *AppState, game_tree *GameTree)
{
    ...
    ClearTraversals(AppState, AppState->DebugTraversalNodes);

    while (CurrentNode)
    {
        s32 CurrentNodeIndex = GetGameTreeIndexFromPointer(AppState, CurrentNode);

        traversal_result FirstChildTraversal = Old_TraverseFirstChild(AppState, AppState->DebugTraversalNodes, CurrentNode);
        ...
        else if (Is_Game_Tree_Index_In_Range(NextSiblingTraversal.GameTreeIndex))
        {
            ...
            SetTraversalAsVisited(AppState->DebugTraversalNodes, CurrentNodeIndex);
        }
        else
        {
    ...
    {
        if (AppState->DebugTraversalNodes[I].Visited)
        {
            Count += 1;
        }
    }
...
{
    s32 GameTreeCount = Debug_WalkGameTreeAndReturnCount(AppState, AppState->RootGameTree);
...
internal void MakeGameTreeTheRoot(app_state *AppState, game_tree *NewRoot)
{
    ...
    game_tree *NewFreeTree = AppState->RootGameTree;
...
internal void AddPotential(app_state *AppState, game_state *GameState, piece Piece, square EndSquare, move_type MoveType)
{
    ...
    */
    ...
    game_tree *GameTree;
    /* NOTE: @Copypasta HandleMove */
    if ((Is_White_Turn(GameState) && WhiteIsInCheck) || (Is_Black_Turn(GameState) && BlackIsInCheck))
    {
    ...
    {
        GameTree = CreateGameTree(AppState);
    }
...
internal void GenerateAllPotentials(app_state *AppState)
{
    ...
    {
        Traversal->CurrentNode = AppState->RootGameTree;
        Traversal->Depth = 1;
    }
    ...
    while (Traversal->CurrentNode && ProcessedNodeCount < MaxNodesToBeProcessed)
    {
        ...
        else if (Is_Game_Tree_Index_In_Range(FirstChildTraversal.GameTreeIndex) && Traversal->Depth < Traversal->MaxDepth)
        {
        ...
        else
        {
            ...
            if (Traversal->Depth == 0)
            {
                ...
                ClearTraversals(AppState, Traversal->Nodes);
...
internal void DrawBoard(app_state *AppState)
{
    ...
    if (CurrentNode)
    {
        ...
        { /* NOTE: Draw score. */
            ...
            sprintf(Buff, "Score: %.4f", AppState->GameTreeCurrent->Score);
            DrawText(Buff, 4.0f, 0.0f, 12.0f, Color);

            sprintf(Buff, "Worst: %.4f", AppState->GameTreeCurrent->WorstFollowingMoveScore);
            DrawText(Buff, 4.0f, 14.0f, 12.0f, Color);


            sprintf(Buff, "Adj_S: %.4f", GetGameTreeAdjustedScore(AppState->GameTreeCurrent));
...
internal void DrawGameTree(app_state *AppState)
{
    ...
    if (IndexIsInRange)
    {
        Vector2 Position = IndexIsInRange ? AppState->DisplayNodes[Index].Position : (Vector2){0.0f, 0.0f};
...
internal void IncrementallySortGameTree(app_state *AppState)
{
    ...
    if (!AppState->RootGameTree)
    {
        return;
    }

    traversal *Traversal = &AppState->SortTraversal;
    game_tree *CurrentRoot = GetRootGameTree(Traversal->CurrentNode);

    if (CurrentRoot == AppState->FreeGameTree || Traversal->CurrentNode == 0)
    {
        Traversal->CurrentNode = AppState->RootGameTree;
...
internal void UpdateDisplayNodes(app_state *AppState)
{
    ...
    Vector2 *Position = &AppState->UpdateCurrentPosition;
    f32 NodeSizePlusPadding = AppState->Ui.GameTreeNodeSize + AppState->Ui.GameTreeNodePadding;
    traversal *Traversal = &AppState->UpdateTraversal;
    game_tree *CurrentRoot = GetRootGameTree(Traversal->CurrentNode);

    if (CurrentRoot == AppState->FreeGameTree || Traversal->CurrentNode == 0)
    {
        Traversal->CurrentNode = AppState->RootGameTree;
        ClearTraversals(AppState, AppState->UpdateTraversal.Nodes);
        AppState->UpdateCurrentPosition = (Vector2){0.0f, 0.0f};
    }
    ...
    while (Traversal->CurrentNode && TraversalCount < MaxTraversalCount)
    {
        ...
        traversal_result FirstChildTraversal = Old_TraverseFirstChild(AppState, AppState->UpdateTraversal.Nodes, Traversal->CurrentNode);
        traversal_result NextSiblingTraversal = Old_TraverseNextSibling(AppState, Traversal->CurrentNode);
        ...
        else if (Is_Game_Tree_Index_In_Range(NextSiblingTraversal.GameTreeIndex))
        {
            ...
            SetTraversalAsVisited(AppState->UpdateTraversal.Nodes, CurrentNodeIndex);
        }
        else
        {
            Traversal->CurrentNode = Traversal->CurrentNode->Parent;
...
internal void DebugDrawFreeTreeCount(app_state *AppState)
{
    ...
    while (CurrentNode)
    {
        Assert(CurrentNode != AppState->RootGameTree);
        Count += 1;
        s32 FirstChildIndex;
        ...
        else if (CurrentNode->Parent && CurrentNode->Parent != AppState->RootGameTree)
        {
            CurrentNode = CurrentNode->Parent;
            SetTraversalAsVisited(AppState->DebugTraversalNodes, CurrentNodeIndex);
        }
...
internal void InitAppState(app_state *AppState)
{
    AppState->RootGameTree = CreateGameTree(AppState);
    ...
    AppState->Ui.GameTreeNodeSize = 4.0f;
    AppState->Ui.GameTreeNodePadding = 2.0f;

    AppState->GameTreeCurrent = AppState->RootGameTree;
    GenerateAllPotentials(AppState); /* TODO: This probably doesn't need to be run here... */
    ClearTraversals(AppState, AppState->DebugTraversalNodes);
    ClearDisplayNodes(AppState);
    AppState->GameTreeCurrent = AppState->RootGameTree;
...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        { /* TODO: delete this debug code */
            char Buff[64];
            sprintf(Buff, "Node pool size %d", Game_Tree_Node_Pool_Size);
            DrawText(Buff, SCREEN_WIDTH - 190, 66, 18, (Color){0,0,0,255});
        }

        Debug_CheckTheCountsOfTheTrees(&AppState);
        Debug_CheckThatTreeDoesNotContainNode(&AppState, AppState.FreeGameTree, AppState.RootGameTree);
